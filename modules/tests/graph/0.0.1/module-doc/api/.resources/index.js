var index = [
{'name': 'tests.graph', 'type': 'package', 'url': 'index.html#section-package', 'doc': '', 'tags': [],'icons': ['icon-package']},
{'name': 'map', 'type': 'function', 'url': 'index.html#map', 'doc': '', 'tags': [],'icons': ['icon-shared-member']},
{'name': 'ColorMapTests.shouldBeWhiteByDefault', 'type': 'function', 'url': 'ColorMapTests.type.html#shouldBeWhiteByDefault', 'doc': '', 'tags': [],'icons': ['icon-shared-member']},
{'name': 'ColorMapTests.shouldSetColor', 'type': 'function', 'url': 'ColorMapTests.type.html#shouldSetColor', 'doc': '', 'tags': [],'icons': ['icon-shared-member']},
{'name': 'ColorMapTests', 'type': 'class', 'url': 'ColorMapTests.type.html', 'doc': '', 'tags': [],'icons': ['icon-class']},
{'name': 'CycleTests.shouldNotBeEmpty', 'type': 'function', 'url': 'CycleTests.type.html#shouldNotBeEmpty', 'doc': '', 'tags': [],'icons': ['icon-shared-member']},
{'name': 'CycleTests.shouldHaveTenVertices', 'type': 'function', 'url': 'CycleTests.type.html#shouldHaveTenVertices', 'doc': '', 'tags': [],'icons': ['icon-shared-member']},
{'name': 'CycleTests.shouldBeTraversedInOrder', 'type': 'function', 'url': 'CycleTests.type.html#shouldBeTraversedInOrder', 'doc': '', 'tags': [],'icons': ['icon-shared-member']},
{'name': 'CycleTests', 'type': 'class', 'url': 'CycleTests.type.html', 'doc': '', 'tags': [],'icons': ['icon-class']},
{'name': 'DistanceMapperTest.testHopDistancesInCycle', 'type': 'function', 'url': 'DistanceMapperTest.type.html#testHopDistancesInCycle', 'doc': '', 'tags': [],'icons': ['icon-shared-member']},
{'name': 'DistanceMapperTest.testHopDistancesInRouteMap', 'type': 'function', 'url': 'DistanceMapperTest.type.html#testHopDistancesInRouteMap', 'doc': '', 'tags': [],'icons': ['icon-shared-member']},
{'name': 'DistanceMapperTest.testDistancesInRouteMap', 'type': 'function', 'url': 'DistanceMapperTest.type.html#testDistancesInRouteMap', 'doc': '', 'tags': [],'icons': ['icon-shared-member']},
{'name': 'DistanceMapperTest', 'type': 'class', 'url': 'DistanceMapperTest.type.html', 'doc': '<p>Test <code><span title=\'graph.traversal.visitor::DistanceMapper\'><span class=\'type-identifier\'>DistanceMapper</span></span></code>.</p>\n', 'tags': [],'icons': ['icon-class']},
{'name': 'VertexPropagatorTests.shouldPropagateToNeighbors', 'type': 'function', 'url': 'VertexPropagatorTests.type.html#shouldPropagateToNeighbors', 'doc': '', 'tags': [],'icons': ['icon-shared-member']},
{'name': 'VertexPropagatorTests', 'type': 'class', 'url': 'VertexPropagatorTests.type.html', 'doc': '', 'tags': [],'icons': ['icon-class']},
{'name': 'EdgePropagatorTests.shouldPropagateViaEdges', 'type': 'function', 'url': 'EdgePropagatorTests.type.html#shouldPropagateViaEdges', 'doc': '', 'tags': [],'icons': ['icon-shared-member']},
{'name': 'EdgePropagatorTests.shouldOrientEdgesOutwards', 'type': 'function', 'url': 'EdgePropagatorTests.type.html#shouldOrientEdgesOutwards', 'doc': '', 'tags': [],'icons': ['icon-shared-member']},
{'name': 'EdgePropagatorTests.shouldOrientEdgesOutwards2', 'type': 'function', 'url': 'EdgePropagatorTests.type.html#shouldOrientEdgesOutwards2', 'doc': '', 'tags': [],'icons': ['icon-shared-member']},
{'name': 'EdgePropagatorTests', 'type': 'class', 'url': 'EdgePropagatorTests.type.html', 'doc': '', 'tags': [],'icons': ['icon-class']},
{'name': 'tests.graph', 'type': 'module', 'url': 'index.html', 'doc': '', 'tags': [],'icons': []}];
var tagIndex = [
];
